...
const FeaturesSection = () => {
  ...
  return (
    <section className="section-padding bg-gradient-to-b from-white to-chakra-gray-50" id="features">
        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {whyFeatures.map((feature, index) => (
              <div 
                key={index}
                className="flex flex-col bg-white rounded-xl p-8 shadow-chakra-md border border-chakra-gray-200 animate-fade-in hover:shadow-chakra-lg transition-all duration-300 hover:-translate-y-1"
                style={{ animationDelay: `${index * 100}ms` }}
              >
                <div className="mb-6 bg-chakra-blue-50 p-4 rounded-lg w-fit">
                  {feature.icon}
                </div>
                <h3 className="text-xl md:text-2xl font-semibold mb-4 text-chakra-gray-800">{feature.title}</h3>
                <p className="text-chakra-gray-600 text-lg leading-relaxed">{feature.description}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
            {tools.map((tool, index) => (
              <div 
                key={index}
                className="bg-white rounded-xl p-8 shadow-chakra-md border border-chakra-gray-200 animate-fade-in hover:shadow-chakra-lg transition-all duration-300 hover:-translate-y-1"
                style={{ animationDelay: `${index * 150}ms` }}
              >
                <div className="flex items-start mb-6">
                  <div className="bg-chakra-blue-600 text-white rounded-full w-12 h-12 flex items-center justify-center font-bold text-xl mr-6 mt-1">
                    {tool.number}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <div>
                        <h3 className="text-2xl md:text-3xl font-semibold flex items-center gap-3">
                          {tool.title}
                          {!tool.available && (
                            <span className="text-xs bg-chakra-gray-100 text-chakra-gray-600 px-3 py-1 rounded-full font-medium">
                              Coming Soon
                            </span>
                          )}
                        </h3>
                    ...
                    <p className="text-chakra-gray-600 mb-6 text-lg leading-relaxed">{tool.description}</p>
                    <ul className="space-y-3">
                      {tool.features.map((feature, featureIndex) => (
                        <li key={featureIndex} className="flex items-center text-lg text-chakra-gray-700">
                          <div className="w-2 h-2 bg-chakra-blue-600 rounded-full mr-4"></div>
                          {feature}
                        </li>
                      ))}
                    </ul>
